...
function TestFindAllPayment() {
	...
	const getAllPayment = async () => {
		...
		await axios.get("http://localhost:3000/payment/getAllPayment")
    ...
    const deletePayment = async () => {
		...
		await axios.delete(`http://localhost:3000/payment/${paySeq}`, req)	
			.then((resp) => {									
                ...
                <div className="admin_container">
                    ...
                    <div>
                            ...
                            <tbody>
                                {
                                    paymentList.map(function(payment, idx) {
                                        return(
                                            <TableRow obj={payment} key={idx} cnt={idx + 1} />
                                        )
                                    })
                                }
                            </tbody>
                        ...
                        <div>
                            ...
                            <Button size={"default"} color={"none"} text={"삭제"} onClick={deletePayment} />
                        </div>
                    </div>
        ...
        <tr>
            {
                <>
                    <td>
                        <Link to={{pathname: `/TestgetOne/${payment.paySeq}`}}>
                            <span className="underline bbs-title">{payment.paySeq}</span>
                        </Link>
                    </td>
